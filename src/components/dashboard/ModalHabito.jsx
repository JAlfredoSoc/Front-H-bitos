...
function ModalHabito({
  ...
  const [subHabitos, setSubHabitos] = useState([]);
  ...
  useEffect(() => {
    if (formulario.periodo === "semanal") {
      const total = Number(
        formulario.frecuencia * formulario.diasSeleccionados.length,
      );
  ...
  useEffect(() => {
    if (habitoEnEdicion) {
      setFormulario({
        ...
        categoria:
          habitoEnEdicion.categoria?._id || habitoEnEdicion.categoria || "",
        ...
        periodo:
          habitoEnEdicion.periodo || habitoEnEdicion.progreso?.periodo || "",
        frecuencia:
          habitoEnEdicion.frecuencia ||
          habitoEnEdicion.progreso?.frecuencia ||
          "",
        ...
        tipoNotificacion:
          habitoEnEdicion.notificacionConfig?.medio || "recordatorio",
      });
  ...
  const validarFormulario = () => {
    ...
    if (
      ...
      formulario.diasSeleccionados.length === 0
    ) {
  ...
  const procesarDatosHabito = () => {
    ...
    const fechaBase =
      formulario.fechaInicio || new Date().toISOString().split("T")[0];
    ...
    return {
      ...
      subHabitos: tipo === "rutina" ? subHabitos : [],
    };
  };
  ...
  const calcularFrecuenciaSemanal = () => {
    if (formulario.periodo === "semanal") {
      return (
        Number(formulario.frecuencia) * formulario.diasSeleccionados.length
      );
    }
    return 0;
  };
          ...
          <div className="modal-body px-4 pb-4">
            ...
            <form onSubmit={manejarSubmit}>
              <div className="row g-3">
                ...
                {tipo === "rutina" && (
                  <div className="mt-3">
                    <label className="fw-semibold">Subhábitos</label>
                    ...
                    <div className="d-flex gap-2 mb-2">
                      <input
                        ...
                        onChange={(e) =>
                          setNuevoSub({ ...nuevoSub, nombre: e.target.value })
                        }
                      />
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (!nuevoSub.nombre || !nuevoSub.descripcion) return;

                          setSubHabitos([...subHabitos, nuevoSub]);
                          setNuevoSub({ nombre: "", descripcion: "" });
                        }}>
                        ➕
                      </button>
                    </div>
                ...
                <div className="col-md-6">
                  ...
                  {errores.periodo && (
                    <div className="invalid-feedback">{errores.periodo}</div>
                  )}
                </div>

                {(formulario.periodo === "semanal" ||
                  formulario.periodo === "diario") && (
                  <div className="col-12">
                    <label className="form-label fw-semibold">
                      Días de la semana *
                    </label>
                ...
                <div className="col-md-6">
                  ...
                  />
                  {errores.frecuencia && (
                    <div className="invalid-feedback">{errores.frecuencia}</div>
                  )}
                </div>

                {formulario.periodo === "semanal" &&
                  formulario.frecuencia > 0 &&
                  formulario.diasSeleccionados.length > 0 && (
                    <div className="alert alert-info mt-2">
                      En total el hábito se repetirá{" "}
                      <strong>{totalRepeticiones}</strong> veces.
                    </div>
                  )}
                  ...
                  {formulario.notificacionActiva && (
                    <div className="mt-2">
                      ...
                      <select
                        ...
                        <option value="interna">Recordatorio (app)</option>
                      </select>

                      {formulario.tipoNotificacion === "telegram" && (
                        <small className="text-muted d-block mt-2">
                          Asegúrate de tener registrado tu número de teléfono en
                          tu perfil
                        </small>
                      )}
